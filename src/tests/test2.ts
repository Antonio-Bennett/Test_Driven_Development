...
export const weightedSort = {
    title: 'Weighted Sort',
    desc: 'Enter a space separated list of numbers. This function sorts them by weight which is the sum of the digits of a number',
    func: (input = '') => {
...
if (import.meta.vitest) {
    ...
    it('returns invalid input types', () => {
        //Numbers
        expect(weightedSort.func(1)).toBe(
            'Invalid Input Type. Expected: "string", Received: "number"',
        );
        expect(weightedSort.func(1.65)).toBe(
            'Invalid Input Type. Expected: "string", Received: "number"',
        );
        expect(weightedSort.func(NaN)).toBe(
            'Invalid Input Type. Expected: "string", Received: "number"',
        );
        //Boolean
        expect(weightedSort.func(true)).toBe(
        ...
        expect(weightedSort.func(undefined)).toBe('');
        //Arrays/Objects <- Arrays are objects
        expect(weightedSort.func({ test: 'test' })).toBe(
            'Invalid Input Type. Expected: "string", Received: "object"',
        );
        expect(weightedSort.func([1, 2, 3])).toBe(
            'Invalid Input Type. Expected: "string", Received: "object"',
        );
        expect(weightedSort.func(() => {})).toBe(
    ...
    it('returns known invalid string', () => {
        ...
        expect(weightedSort.func()).toBe('');
        expect(weightedSort.func('')).toBe('');
        expect(weightedSort.func('t')).toBe('"t" is not a number');
        expect(weightedSort.func('8 16 124 x 7 91')).toBe(
            '"x" is not a number',
        );
        expect(weightedSort.func('8,16,124,12,7,91')).toBe(
    ...
    it('sorts list by weight', () => {
        ...
        expect(weightedSort.func('56 65 74 100 99 68 86 180 90')).toBe(
            '100 180 90 56 65 74 68 86 99',
        );
        expect(weightedSort.func('1 17 9 4 20')).toBe('1 20 4 17 9');
        expect(weightedSort.func('1')).toBe('1');
        expect(weightedSort.func('8 26')).toBe('26 8');
    });
}
